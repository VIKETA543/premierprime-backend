...
router.post('/listproduct', cors({ origin: '*' }), async (req, res) => {
  res.header('Access-Control-Allow-Origin', '*'); // Allow all origins, or specify a specific origin
  res.header('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS'); // Allow specified methods
  res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept'); // Allow specified headers
  await pool.connect().then(async (r) => {
    ...
    if (r._connected) {
      try {
        ...
        if (data.store_number) {
          query = "SELECT store_products.product_number,products.name,products.image,store_products.product_category,products.imageurl,products.description,prodcart.category_name,store_products.date_created FROM store_products LEFT JOIN products ON store_products.product_number=products.serialnumber  LEFT JOIN prodcart ON store_products.product_category=prodcart.serialnumber WHERE store_products.store_number=$1"
          r.query(query, [data.store_number], (error, results) => {
            if (error) {
              ...
              if (results.rows.length > 0) {
                ...
                r.release()
                return res.status(200).json({ message: "No products found for the specified store" })
              }
            }
